...
forecast_xts <- xts(x= pforecast, frequency = 365.25,
                    order.by = data62)
...
##################################
# 20230120 Toevoegingen tbv eindpresentatie
##################################


### 2003 Visual ts totaledataset
Dagpax.ts2003 <- ts(Dagpax$PassagiersPerDag, start = c(2003,214), frequency = 365.25)
autoplot(Dagpax.ts2003, xlab="Jaren", ylab="Aantal passagiers", main="Aantal passagiers 1-8-2003 t/m 31-8-2021")
#dc2003 <- decompose(Dagpax.ts2003)
#autoplot(dc2003) 


#Toegevoegd DyGraph van 1 aug 2003 tm 31 aug 2021 
data_xts.dp2003 <- xts(x= Dagpax$PassagiersPerDag, frequency = 365.25,
                  order.by = Dagpax$Actuele.datum.tijd)

dygraph (data_xts.dp2003, main = "Aantal passagiers 1-8-2003 t/m 31-8-2021",
         ylab="Aantal passagiers")  %>% 
  dyOptions(stackedGraph = TRUE) %>%
  dyRangeSelector(height = 20)


# Aanroep NeuralNet met 2 repeats en 1 hidden layer, dataset TS 2003-2021
#calc_nnet(2,1,Dagpax.ts2003)  #NNAR(36,1,19)[365] -> 5625 (!!!)

# Selectie 2003-08-01 t/m 2021-08-31 -> 5625(!!!)
# 

### Neuralnet voor periode 1-8-2003 tm 31-8-2021 (hele dataset)
# Aanroep NeuralNet met 2 repeats en 1 hidden layer, dataset TS 2003-2021

RNGversion("3.5.2") 
set.seed(12345)

nnet2003 <- nnetar (Dagpax.ts2003, 2, P = 1) 
nnetforecast2003 <- forecast(nnet2003, h = 62, PI = T)  # PI = Prediction intervals

### Data, Fitted en Forecast in 1 afbeelding; warnings genegeerd
autoplot(Dagpax.ts2003, series = 'Data', 
         xlab="Jaren",
         ylab="Aantal passagiers", 
         main="Aantal passagiers 1-8-2003 t/m 31-8-2021 (werkelijk, model, voorspelling)",
         color = 'blue') + 
  autolayer(fitted(nnet2003), series = 'Fitted', color = 'purple') +
  autolayer (nnetforecast2003, series = 'Forecast', color = 'darkblue') #+ xlim(2019, 2021)



### Controle: Check voorspelling 16 aug - 31 aug 2021, dus na corona 
Dagpax_20210815 <- Dagpax %>% filter(Actuele.datum.tijd >= '2003-08-01' & Actuele.datum.tijd <='2021-08-15') #909 obs
summary(Dagpax_20210815)

Dagpax.ts20210815 <- ts(Dagpax_20210815$PassagiersPerDag, start = c(2003,214), frequency = 365.25)
#autoplot(Dagpax.ts20210815, xlab="Jaren", ylab="Aantal passagiers", main="Aantal passagiers 1-8-2003 t/m 15-8-2021")
#dc20210815 <- decompose(Dagpax.ts20210815)
#autoplot(dc20210815) 


nnet200210815 <- nnetar (Dagpax.ts20210815, 2, P = 1) 
nnetforecast200210815 <- forecast(nnet200210815, h = 16, PI = T)  # PI = Prediction intervals

### Data, Fitted en Forecast in 1 afbeelding; warnings genegeerd
autoplot(Dagpax.ts20210815, series = 'Data', 
         xlab="Jaren",
         ylab="Aantal passagiers", 
         main="Aantal passagiers 1-8-2003 t/m 15-8-2021 (werkelijk, model, voorspelling 16 dagen)",
         color = 'blue') + 
  autolayer(fitted(nnet200210815), series = 'Fitted', color = 'purple') +
  autolayer (nnetforecast200210815, series = 'Forecast', color = 'yellow') #+ xlim(2020, 2021)


# Vergelijking van voorspelling 16-8-21 tm 31-8-21 met werkelijke waarde
aug_nnet <- round(as.data.frame(nnetforecast200210815$mean))  #voorspellingen van nnet naar DF
#head(aug_nnet)

werkelijk_pax <- as.data.frame(tail(Dagpax_20210815, 16)[,2])   #Werkelijke waarde pax
tmp <- cbind(werkelijk_pax, aug_nnet, abs(werkelijk_pax - aug_nnet))
res <- round (mean (tmp[,3]))
paste("-> gemiddelde afwijking", res) # Gemiddelde afwijking is (slechts) 1085

### Omdat gemiddelde afwijking na corona relatief klein is, 
### zal het model met recentere data alsnog relatief goed blijven voorspellen.

######  EINDE :-) ######

### Waarschijnlijk nu gewenst om data_xts.dp2003 te exporteren naar PowerBI
