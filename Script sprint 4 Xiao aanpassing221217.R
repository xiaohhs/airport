...
library(forecast)
library(ggplot2)
library(stats) # nodig voor hold-winters
...
#==========================================================
 ...
 #SES model (Simple Exp. Smoothing)
library(TSstudio) # nodig voor SES model
library(forecast) # nodig voor SES model
library(plotly) # nodig voor SES model
# vindt alle waarden even belangrijk, terwijl juist de pieken belangrijk zijn
...
voorsp_60 <- as.data.frame(fc_ses$mean)  #voorspellingen van TS naar DF
head(voorsp_60)
# Afronden
voorsp_60round <- round(voorsp_60) %>%
  rename(Forecast = x)
...
voorsp_60 <- as.data.frame(fc_hw$mean)  #voorspellingen van TS naar DF
head(voorsp_60)
# Afronden
voorsp_60round <- round(voorsp_60) %>%
  rename(Forecast = x)
...
# DF werkelijk en voorspellingen samenvoegen en nieuwe kolom om verschil te laten zien
Dagpax_mrtjun19 <- Dagpax2 %>%
  filter(Actuele.datum.tijd >= "2019-03-02" & Actuele.datum.tijd <= "2019-06-29")

Dagpax_forecast <- cbind(Dagpax_mrtjun19, voorsp_60round) %>%
  mutate(verschil = (PassagiersPerDag - Forecast))
head(Dagpax_forecast)
summary(Dagpax_forecast$verschil)
mean(abs(Dagpax_forecast$verschil)) #1588 foutmarge, komt door 'smoothing
# neemt daarom de outliers niet goed mee. Je kunt window aanpassen
