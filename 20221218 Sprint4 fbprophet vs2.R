...
# Runt wel, maar blijkbaar zonder seasonality
future <- make_future_dataframe(m, 60, freq = 'd') # voorspelt 60 dagen, dus vanaf 1-7-19
forecast <- predict(m, future)
plot(m, forecast)
dyplot.prophet(m, forecast)

head(forecast$ds) #begint op 1-8-03
tail(forecast$ds) #eindigt op 29-8-19
...
df_forecast <- forecast %>%
  ...
  select(yhat) %>%
  rename(Forecast = yhat)
df_forecastround <- round(df_forecast)

df_verschil <- cbind(df_werkelijk, df_forecastround) %>%
  mutate(verschil = (PassagiersPerDag - Forecast)) 
head(df_verschil)
summary(df_verschil$verschil)
#    Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
# -61.0   987.5  1408.5  1352.5  1801.2  2370.0 
...
########
m2 <- prophet(Dagpax_prophet, seasonality.mode = 'multiplicative',daily.seasonality=TRUE)
future <- make_future_dataframe(m2, 60, freq = 'd') # voorspelt 60 dagen, dus vanaf 1-7-19
...
summary(df_verschil$verschil)
#Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
#-116.4   924.7  1330.7  1274.5  1711.0  2285.0 

# Verschil: Gemiddelde (absolute) aantallen per dag
mean(abs(df_verschil$verschil)) #1278
# Conclusie; voorspelling is veel minder accuraat dan ARIMA
